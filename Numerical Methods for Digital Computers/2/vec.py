...
def add(u,v):
    ...
    """
    assert u.D == v.D
    return Vec({k for k in u.D},{k:getitem(u,k)+getitem(v,k) for k in u.D})
...
def scalar_mul(v, alpha):
    ...
    True
    """
    return Vec({k for k in v.D},{k:alpha*getitem(v,k) for k in v.D})
...
def neg(v):
    ...
    True
    """
    return Vec({k for k in v.D},{k:-1*getitem(v,k) for k in v.D})
